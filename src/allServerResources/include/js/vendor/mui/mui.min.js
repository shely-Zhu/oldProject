...
function(a) {
    ...
    a.fn.on = function(b, d, e, mdObj) {

        //md 为新增的方法，作为on方法的第四个参数，用于点击的同时发送埋点请求
        //mui在这里给所有on事件绑定方法，此时页面上的js逻辑已经走到了业务代码里
        //判断是否为mdClick方法，如果是，执行e回调函数的逻辑，不执行下面的return逻辑
        if( b == 'mdClick'){


            //正常的tap请求
            // a("body").on('tap', d, e);
            a("body").on('tap', d, function() {
                setTimeout(() => {
                    e
                }, 500)
            });

            //给d添加tap方法，回调函数为e，再添加一次tap方法，回调函数为埋点请求
            if( mdObj ){
                a("body").on('tap', d, function(){
                    window._submitMd && window._submitMd( 3, mdObj.htmdEvt, mui(this)[0]);
                });
            }
            ...
            b.addEventListener("focusin", function(h) {
                if (!(a.os.plus && window.plus && plus.webview.currentWebview().children().length > 0)) {
                    ...
                    // if (i.tagName && ("TEXTAREA" === i.tagName || "INPUT" === i.tagName && ("text" === i.type || "search" === i.type || "number" === i.type))) {
                        // 去掉type=text;为解决实名认证中的点击图文验证码会跳转到顶部的bug
                    if (i.tagName && ("TEXTAREA" === i.tagName || "INPUT" === i.tagName && ( "search" === i.type || "number" === i.type))) {
    ...
    a.init = function(b) {
        ...
        a.ready(function() {
            a.doAction("inits", function(b, d) {
                ...
                //修改，判断当前是否为上拉加载，使可以多次初始化
            	if( d.name != 'pullrefresh' ){
      ...
      , l = a.Class.extend({
        ...
        handleEvent: function(b) {
            ...
            case "scrollend":
                ...
                if( (0-b.detail.lastY)  ){
                    //修改：有向上滚动的距离, 显示回到顶部按钮
                    b.detail.wrapper.querySelector('.goTopBtn').style.display = 'block';
                }
        ...
        _scrollend: function(b) {
            //修改，添加回到顶部时，回到顶部按钮消失功能
            //b.detail.options.scrollY为了限制左右滑屏时触发
            if( b.detail.y == 0 && b.detail.options.scrollY ){
                //按钮消失
                b.target.parentNode.querySelector('.goTopBtn').style.display = 'none';
            }
      ...
      , g = a.Scroll.extend(a.extend({
        handleEvent: function(a) {
            ...
            if( !!a.target.className ){
                if( a.target.getAttribute('class').indexOf('goTopBtn') != -1){
                    this.y = 0;
                    this.lastY = 0;
                }
            }
        ...
        pullupLoading: function(a, b, c) {
            ...
            // this.scrollTo(b, this.maxScrollY, c, this.options.bounceEasing), //禁用掉返回顶部高度
...
function(a, b, c) {
    ...
    k.prototype.init = function() {
        ...
        //修改，调整开关点击后的样式
        this.handleX = this.toggleWidth - this.handleWidth 
    }
    ,
                ...
                a.each(["keyup", "change", "input", "focus", "cut", "paste"], function(a, c) {
                    !function(a) {
                        b.addEventListener(a, function() {
                        	if(f.parentElement.parentElement.parentElement.className.indexOf("formBox")==-1){
