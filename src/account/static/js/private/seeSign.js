                    ...
                    if (json.data == '') { // 没有记录不展示
                        ...
                        $.each(dataList, function(i, el) {
                          ...
                          if (el.contracturl.indexOf(".pdf") != -1) {
                              el.line = true; //线上可预览
                              el.href = site_url.frontendMountDownload_api + "?url=" + el.contracturl + "&name=" + new Base64().encode(el.modulename) + "&show=1";
                          } else {
                              el.line = false; //需下载
                              el.href = site_url.frontendMountDownload_api + "?url=" + el.contracturl + "&name=" + new Base64().encode(el.modulename);
                          }
                        })
                    }
                    setTimeout(function () {
