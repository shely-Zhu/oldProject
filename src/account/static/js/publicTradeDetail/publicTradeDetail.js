...
$(function () {
    var obj = {
        ...
        gV: { // 全局变量
            ...
            //test
            allotType: "0",//交易类别 0：购买 1：赎回 2：定投, 3：分红
            isBuy: splitUrl()['isBuy'],//是否为现金宝购买
            isCash: false,//是否为现金宝
        },
        init: function () {
        ...
        getFundTradeDetail: function () {
            ...
            var obj = [{
                ...
                callbackDone: function (json) {
                    ...
                    if ("1" != json.data.tradeStatus || "2" != json.data.debitStatus){
                        //如果是失败状态 将状态文案变为红色
        ...
        showFundStatus: function (isBuy, model) {
            ...
            if (isBuy){
                ...
                $('.header .amount').html(model.tradeShares + "份");//交易申请份额
            }
            ...
            if (isBuy){
                ...
                $('.buy_info .item_1').html(model.tradeAmount);//赎回产品
                $('.buy_info .item_2').html(model.tradeAmount);//赎回份额
                $('.buy_info .bank_icon').attr('url', model.bankLogoUrl);//到账银行卡icon
                $('.buy_info .item_3').html(model.bankName + model.bankAccount.substring(model.bankAccount.length - 4));//到账银行卡描述
                $('.buy_info .item_4').html(model.tradeAmount);//赎回时间
            }
