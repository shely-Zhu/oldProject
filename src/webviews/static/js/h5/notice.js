//function getNoticeId(name){
	...
	var notice = {
        ...
        getData:function(){
    		...
    		$.ajax({
    		    url: http_url + "/api/app/index/content/h5/h5FindContentDetailById.action",
    		    data: 
    		    	JSON.stringify({
					    "hmac":"", //预留的加密信息
					    "params":{//请求的参数信息
							"contentId":that.contentId,//内容ID
						}
    				}),
    		    type: 'POST',
    		    async : true,
    		    ...
    		    success: function (data) {
    		    	...
    		    	if (data.status == "0") {//成功
                        ...
                        $("#time").html(result.releaseDate);//releaseDate
                        $("#resource").html(result.reportSource);//source
