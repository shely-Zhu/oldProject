		...
		getData: function(){
			...
			var obj = [{
				...
				callbackDone: function(json){
					...
					//机构类型
					if(!!jsonData.orgType){
						$('[check=orgTypeSelect]').html(jsonData.orgTypeDesc).attr('num',jsonData.orgType).addClass('unable noIcon');
					}
					//出生日期  ---	
					if( !!jsonData.birthday ){
						$('[check=birthSelect]')
							.html( jsonData.birthday.substring(0,4)+"-"+jsonData.birthday.substring(4,6)+"-"+jsonData.birthday.substring(6,8) )
							.attr("num",1).addClass('unable noIcon');
					}
					//代理人姓名
		...
		eventsFunc: function(){
				...
				var obj=[{
					...
					data: {    
						...
						params:{//请求的参数信息 
							...
							orgType:$("[check=orgTypeSelect]").attr("num"),
			...
			mui("body").on('tap','.investSelect',function(){
				if( !$(this).hasClass('unable')){
					shipFnc(realName,'仅用于按照监管要求收集贵单位的实际控制人、受益人信息，不影响收益分配，收益仍将分配至贵单位账户','investSelect',['本机构','其他主体'],200);
				}
			})
